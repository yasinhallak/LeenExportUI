<template>
  <div id="data-list-list-view" class="data-list-container">
    ...
    <vs-table ref="table" multiple v-model="selected" pagination :max-items="itemsPerPage" search :data="requests">
...
<!--        </div>-->
        ...
        <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 mr-4 items-per-page-handler">
          <div class="p-4 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
            <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ requests.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : requests.length }} of {{ queriedItems }}</span>
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="status">حالة الطلب</vs-th>
        ...
        <vs-th sort-key="createdDate">تاريخ إنشاء الطلب</vs-th>
        <vs-th>الأوامر</vs-th>
      </template>
      ...
      <template slot-scope="{data}">
        ...
        <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
          ...
          <vs-td>
            <p class="product-name font-medium truncate">{{$t('requestStatus.' + tr.status)}}</p>
          </vs-td>
          <vs-td>
...
import moduleDataList from '@/store/vendor-request/moduleVendorRequest.js'
...
export default {
  ...
  computed: {
    ...
    queriedItems () {
      return this.$refs.table ? this.$refs.table.queriedResults.length : this.requests.length
    }
  },
  methods: {
    ...
    deleteRecord(){
      ...
      this.$store.dispatch('request/removeItem', this.removeItem)
  ...
  created () {
    if (!moduleDataList.isRegistered) {
      this.$store.registerModule('request', moduleDataList)
      moduleDataList.isRegistered = true
    }
    this.$store.dispatch('request/fetchDataListItems')
  },
  mounted () {
    this.isMounted = true
  }
