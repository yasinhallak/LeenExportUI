  ...
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    ...
    <component :is="scrollbarTag" class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      ...
      <div class="p-6">
        ...
        <!-- NAME -->
        <vs-input label="اسم التصنيف الفرعي" v-model="name" class="mt-5 w-full" name="name" v-validate="'required'" />
...
export default {
  ...
  computed: {
    ...
    categoryTypes () {
      return this.$store.state.subCategory.categoryTypes
    },
  },
  methods: {
    initValues () {
      ...
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          if (this.dataId !== null && this.dataId >= 0) {
            this.$store.dispatch('subCategory/updateItem', obj).catch(err => { console.error(err) })
          } else {
            delete obj.id
            // obj.popularity = 0
            this.$store.dispatch('subCategory/addItem', obj).catch(err => { console.error(err) })
          }
    ...
    changeSeasonsTypes(){
      ...
      this.$store.dispatch('subCategory/fetchCategoryItems',obj)
